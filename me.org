...
** Auto-Wrapping for Prose

For reflowed text (like in prose modes)
...
** Persist custom file into not oblivion
...
(use-package meow
  :straight t
  :demand t  ; Load immediately to avoid defer issues
  :config
  (defun meow-setup ()
    "Default Meow QWERTY setup."
    (setq meow-cheatsheet-layout meow-cheatsheet-layout-qwerty)
    ;; Motion mode keys (single keys only; no multi-key issues)
    (meow-motion-define-key
     '("j" . meow-next)
     '("k" . meow-prev)
     '("<escape>" . ignore))
    ;; Leader mode (SPC prefix; single keys for digits/help)
    (meow-leader-define-key
     ...
     '("?" . meow-cheatsheet))
    ;; Normal mode keys (all single keys; avoids non-prefix errors like ] d)
...
*** Youtube Link
Custom YouTube links like =[[youtube:VIDEO_ID][Description]]=
Opens =https://www.youtube.com/watch?v=VIDEO_ID= in browser
...
#+begin_src emacs-lisp :tangle "modules/cloutlu-prog.el" :mkdirp yes
...
** Zen-2 Mode
This is a separate, Doom-inspired distraction-free mode. Centers text, hides UI, scales fonts, and uses variable-pitch for readability.

#+begin_src emacs-lisp :tangle "modules/cloutlu-zen-2.el" :mkdirp yes
